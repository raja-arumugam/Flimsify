...
import com.example.moviesdataapp.data.model.SimilarTv
...
class SimilarTvAdapter(private val list: List<SimilarTv>) :
    RecyclerView.Adapter<SimilarTvAdapter.MyViewHolder>() {
    ...
    override fun onCreateViewHolder(
        parent: ViewGroup,
        viewType: Int
    ): SimilarTvAdapter.MyViewHolder {
        binding = ItemSimilarLayoutBinding.inflate(LayoutInflater.from(parent.context), parent, false)

        return MyViewHolder(binding)
    }

    override fun onBindViewHolder(holder: SimilarTvAdapter.MyViewHolder, position: Int) {
    ...
    inner class MyViewHolder(val binding: ItemSimilarLayoutBinding) :
        ...
        fun bind(item: SimilarTv) {
            ...
            binding.tvMovieName.text = item.original_name
